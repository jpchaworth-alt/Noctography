...
/* Grids baked from David Lorenz's 2025 light pollution atlas, itself modelled from
   NASA/NOAA VIIRS night lights. Pixel values are (22.0 - sky brightness) x 20, so byte 0
   is a pristine 22.0 mag/arcsec2.
   These two are now the FALLBACK, not the main source: the tile atlas below covers the whole world
   at 1.5 arcmin and is tried first. They stay because they are the only sky brightness that needs
   no network at all, so a phone that has never been online in a region still gets an answer.
   The scandi key is kept in the lookup order and deliberately left unbaked: the colour-inversion
   reconstruction it was waiting for is no longer needed now the atlas's own numbers are available
   per tile, and anywhere it would have covered is served at full resolution by the tiles. */
const ATLAS={
...
/* ============================ 1.5 arcmin tile atlas ============================ */
/* The baked grids above are the fallback now. The real source is 5-degree PNG tiles at the atlas's
   own 1.5 arcmin, baked from David Lorenz's public 2025 binary tiles and served from this app's own
   hosting: 1,032 tiles wherever anything is lit, and 984 open-ocean tiles reduced to a single byte
   in the manifest so they cost nothing. Same encoding as the baked grids, same nearest-cell read,
   so nothing downstream knows the difference.
   Why tiles and not one more baked image: 1.5 arcmin worldwide is 4.5 MB, which is not going into a
   bundle a phone loads at a trailhead. Tiles are fetched for the 150 km ring the horizon diagram
   samples, kept in the Cache API, and read from memory after that, so a second visit and every
   offline visit are free. Miss the cache with no signal and the lookup falls through to the baked
   world grid exactly as before: coarser, never broken. */
const ENGINE_SRC=(document.currentScript&&document.currentScript.src)||'';
const TILES={year:2025,deg:5,w:200,h:200,cacheName:'noct-atlas-2025',
  manifest:null,pngSet:null,mem:new Map(),inflight:new Map(),base:null,
  status:'idle',promise:null,fetched:0,onready:null};
function atlasBase(){
  if(TILES.base) return TILES.base;
  const b=ENGINE_SRC?ENGINE_SRC.replace(/[?#].*$/,'').replace(/[^/]*$/,''):'';
  return TILES.base=b+'atlas/'+TILES.year+'/';
}
async function tileCache(){
  if(typeof caches==='undefined') return null;
  try{ return await caches.open(TILES.cacheName); }catch(e){ return null; }
}
/* Cache first, always. Offline this is the whole story; online it means a tile is paid for once
   ever rather than once per visit. */
...
function loadTileManifest(){
  if(TILES.promise) return TILES.promise;
  TILES.status='loading';
  return TILES.promise=(async()=>{
    try{
      const j=await (await cachedFetch(atlasBase()+'manifest.json')).json();
      TILES.manifest=j;
      TILES.pngSet=new Set(j.png||[]);
      Object.keys(j.flat||{}).forEach(k=>TILES.mem.set(k,j.flat[k]));
      TILES.status='ready';
      return j;
    }catch(e){ TILES.status='unavailable'; return null; }
  })();
}
async function decodeTilePng(blob){
  const w=TILES.w,h=TILES.h;
  const read=cx=>{
  ...
  return new Promise((res,rej)=>{
    ...
    img.onload=()=>{
      try{
        ...
        const cx=cv.getContext('2d',{willReadFrequently:true});
        cx.drawImage(img,0,0);
        const out=read(cx); URL.revokeObjectURL(url); res(out);
      }catch(e){ URL.revokeObjectURL(url); rej(e); }
    };
    img.onerror=()=>{ URL.revokeObjectURL(url); rej(new Error('tile decode failed')); };
    img.src=url;
  });
}
/* A decoded tile is 40 KB in memory and someone planning a trip can wander across a lot of them.
   Map keeps insertion order, so dropping from the front is oldest-first; the one-byte flat entries
   are left alone because they cost nothing and re-reading them costs a manifest lookup. */
function tileTrim(cap){
  cap=cap||200;
  const heavy=[...TILES.mem.keys()].filter(k=>{const v=TILES.mem.get(k);return v&&typeof v!=='number';});
  for(let i=0;i<heavy.length-cap;i++) TILES.mem.delete(heavy[i]);
}
function tileIndex(lat,lon){
  if(lat<-65||lat>=75) return null;
  const x=Math.floor(((((lon+180)%360)+360)%360)/TILES.deg)+1;
  const y=Math.floor((lat+65)/TILES.deg)+1;
  return {x:Math.min(72,x),y:Math.min(28,y)};
}
function tileFetch(x,y){
  const k=x+'_'+y;
  if(TILES.mem.has(k)) return Promise.resolve(TILES.mem.get(k));
  if(TILES.inflight.has(k)) return TILES.inflight.get(k);
  if(!TILES.manifest) return Promise.resolve(null);
  if(TILES.pngSet&&!TILES.pngSet.has(k)){ TILES.mem.set(k,null); return Promise.resolve(null); }
  const p=(async()=>{
    try{
      const r=await cachedFetch(atlasBase()+'atlas'+TILES.year+'_'+k+'.png');
      const px=await decodeTilePng(await r.blob());
      TILES.mem.set(k,px); TILES.fetched++;
      tileTrim();
      return px;
    }catch(e){
      return null;   // no signal, no tile: deliberately not remembered, so it retries with signal
    }finally{ TILES.inflight.delete(k); }
  })();
  TILES.inflight.set(k,p);
  return p;
}
/* Fetch every tile the 150 km sampling ring can touch, so atlasSky can stay synchronous: it is
   called eighty-odd times per render by the horizon diagram alone. Stepping the span in 4-degree
   increments rather than sampling corners means no 5-degree tile can fall between samples. */
async function loadTiles(lat,lon,radiusKm){
  lat=lat==null?state.lat:lat; lon=lon==null?state.lon:lon;
  if(lat==null||lon==null) return {status:'idle'};
  if(!await loadTileManifest()) return {status:TILES.status};
  const R=radiusKm||160;
  const dLat=R/111.32, dLon=R/(111.32*Math.max(0.12,Math.abs(cos(lat))));
  const span=(c,d)=>{const out=[];for(let v=c-d;v<c+d;v+=4)out.push(v);out.push(c+d);return out;};
  const want=new Set();
  span(lat,dLat).forEach(la=>span(lon,dLon).forEach(lo=>{
    const t=tileIndex(clamp(la,-64.99,74.99),lo);
    if(t) want.add(t.x+'_'+t.y);
  }));
  const keys=[...want].filter(k=>!TILES.mem.has(k));
  if(!keys.length){ return {status:'ready',tiles:want.size,fetched:0}; }
  let i=0, late=false;
  const run=async()=>{ while(i<keys.length){ const [x,y]=keys[i++].split('_'); await tileFetch(+x,+y); } };
  const all=Promise.all([run(),run(),run(),run()]).then(()=>{
    updateSky();
    if(late&&typeof TILES.onready==='function') TILES.onready();
  });
  /* A first visit to a region is of the order of 100 KB, which is the same shape of wait as the
     forecast. Past seven seconds the app carries on with whatever landed and picks up the rest
     through onready, rather than holding the whole screen for a slow connection. */
  await Promise.race([all,new Promise(r=>setTimeout(()=>{late=true;r();},7000))]);
  updateSky();
  return {status:'ready',tiles:want.size,fetched:TILES.fetched};
}
function tileSky(lat,lon){
  lon=((((lon+180)%360)+360)%360)-180;
  const t=tileIndex(lat,lon);
  if(!t) return null;
  const v=TILES.mem.get(t.x+'_'+t.y);
  if(v==null) return null;
  const label=TILES.year+' 1.5 arcmin tile';
  if(typeof v==='number') return {sqm:22.0-v/20,source:'tile',label};
  const lat0=TILES.deg*(t.y-1)-65, lon0=TILES.deg*(t.x-1)-180;
  const bx=clamp(Math.floor((lon-lon0)/TILES.deg*TILES.w),0,TILES.w-1);
  const by=clamp(Math.floor((lat0+TILES.deg-lat)/TILES.deg*TILES.h),0,TILES.h-1);
  return {sqm:22.0-v[by*TILES.w+bx]/20,source:'tile',label};
}
function tileStatus(){
  const m=TILES.manifest;
  return {status:TILES.status,fetched:TILES.fetched,
          held:[...TILES.mem.values()].filter(v=>v&&typeof v!=='number').length,
          total:m?m.totals.tiles:null,year:TILES.year};
}
function atlasSky(lat,lon){
  const t=tileSky(lat,lon);
  if(t) return t;
  for(const k of ['uk','scandi','world']){
    const L=ATLAS[k];
...
/* Thin cloud and haze do not simply block the sky: they scatter town glow and moonlight
   ...
   Only HIGH cloud belongs here. Mid and low cloud do not scatter glow into your shot in any way
   you would notice, because they have already blocked the stars behind them: they do one job,
   blocking, and that is clearFraction's business rather than this. */
const NATURAL_SKY=22.0;   // natural airglow floor, mag/arcsec2
function airFactors(c){
  if(!c) return {veil:0,hum:0};
  const veil = (c.high==null)
    ? clamp(0.40*(c.total||0)/100,0,1)
    : clamp(0.75*(c.high||0)/100,0,1);
...
/* Low and mid cloud is what actually stops a shot. High cirrus dims and softens, and a night
   ...
   is usable reads this rather than total cover. Layers are combined the same way the forecast
   reader combines them, by taking the worst, not by adding overlapping sheets together. */
function lowMidCover(c){
  if(!c) return null;
...
/* ZHR assumes a perfect observer watching the whole sky. Real people, even experienced ones,
   catch roughly three quarters of that, so every meteor rate is scaled by this perception factor.
   It applies to meteors, and to nothing else. A satellite pass is a known object on a known track
   at a known time: either it is above your horizon and lit or it is not, and no perception factor
...
function computeNight(y,m,d){
  ...
  for(let t=startL.getTime();t<=endL.getTime();t+=STEP*60000){
    ...
    if(s.sunAlt<-6){
      ...
      if(cl){
        ...
        /* dew is worked out after the loop: see the note below */
      }
  ...
  /* Dew only matters while it is dark. This used to scan everything from six degrees of twilight
     onwards, which meant a dew point reached in the morning, after the last useful hour, produced a
     "pack the dew straps" warning for a night that was already over. Restricted to the hours the
     app itself calls darkness, and to the evening side of them: dew forming before it is properly
     dark is still worth knowing about, dew forming after dawn is not. */
  {
    const usable = slots.filter(s => s.cloud && s.cloud.temp != null && s.cloud.dew != null);
    let last = -1;
    usable.forEach((s, i) => { if (s.sunAlt < -15) last = i; });
    let win = last >= 0 ? usable.slice(0, last + 1).filter(s => s.sunAlt < -12) : [];
    /* Midsummer at these latitudes never reaches -15 at all, so fall back to nautical twilight
       rather than going silent on the shortest nights, which are exactly when dew is worst. */
    if (!win.length) win = usable.filter(s => s.sunAlt < -10);
    dewGap = null; dewAt = null;
    win.forEach(s => {
      const gap = s.cloud.temp - s.cloud.dew;
      if (dewGap == null || gap < dewGap) { dewGap = gap; dewAt = s.t; }
    });
  }
...
function tonight(){
  ...
  let best = null;
  let noCloudData = false;
  usable.forEach(s => {
    const cf = clearFraction(s.cloud);
    if (cf == null) noCloudData = true;
    /* No data is not good news. This used to assume 75% clear, which quietly flattered a whole run
       of nights whenever a feed failed. Neutral instead, and the night says on its face that
       nothing is known rather than looking like a good one. */
    const clear = cf == null ? 0.5 : cf;
  ...
  /* Low and mid cloud across the dark hours: the figure the grade and the window are judged on. */
  const lmVals = usable.map(s => lowMidCover(s.cloud)).filter(v => v != null);
  const lmMean = lmVals.length ? lmVals.reduce((a, b) => a + b, 0) / lmVals.length : null;

  let win = null, hedge = null, meanQ = 0;
  if (best){
    ...
    /* A window has to clear an absolute bar, not merely be the best of a bad night. If the best
       stretch is still half covered in low or mid cloud there is nothing worth aiming for, and it
       becomes a hedge rather than a plan: the least bad hour, offered as such. */
    const wl = usable.slice(a, b + 1).map(s => lowMidCover(s.cloud)).filter(v => v != null);
    const cover = wl.length ? wl.reduce((x, y) => x + y, 0) / wl.length : null;
    const cand = { from: usable[a].t, to: usable[b].t, quality: best.quality, cover };
    if (cover != null && cover > 50) hedge = cand; else win = cand;
  }

  /* Star trails need an unbroken run, not an average: four scattered clear hours are worth nothing
     if no single stretch holds long enough to stack. Under 10% low and mid, read off an hourly
     forecast, so the answer is honest to about an hour either way. */
  let clearRun = null;
  let runStart = null, prevSlot = null;
  usable.forEach((s, i) => {
    const c = lowMidCover(s.cloud);
    const ok = c != null && c < 10;
    if (ok && runStart == null) runStart = s.t;
    if (!ok && runStart != null && prevSlot){
      const mins = (prevSlot.t - runStart) / 60000 + STEP;
      if (!clearRun || mins > clearRun.mins) clearRun = { from: runStart, to: prevSlot.t, mins };
      runStart = null;
    }
    if (ok && runStart != null && i === usable.length - 1){
      const mins = (s.t - runStart) / 60000 + STEP;
      if (!clearRun || mins > clearRun.mins) clearRun = { from: runStart, to: s.t, mins };
    }
    prevSlot = s;
  });
  ...
  /* The forecast gives the strongest gust within each hour, so taking the strongest of those
     across a nine-hour night described the whole evening by its single worst second: one squall at
     04:00 made a calm evening read as rough. Quote what it is typically doing over the hours you
     would actually be out, and carry the peak separately for when it stands well above that. */
  const gs = gusts.slice().sort((a, b) => a - b);
  const typicalGust = gs.length ? gs[Math.floor(gs.length * 0.6)] : null;
  const peakGust = gs.length ? gs[gs.length - 1] : null;
  const wind = winds.length ? { mean: winds.reduce((a, b) => a + b, 0) / winds.length,
    max: Math.max.apply(null, winds), gust: typicalGust, peakGust,
    gustSpike: typicalGust != null && peakGust != null && peakGust >= typicalGust + 12 } : null;
  ...
  /* Cloud ceiling on the grade. A night averaging more than a third low or mid cloud cannot be
     described as great however good its single best hour looks, and past 70% it is written off.
     The cap sits on top of the score rather than replacing it, so a night that is clear early and
     clouds over still shows its good early hours: it simply cannot be called great overall. */
  let cloudCap = null;
  if (lmMean != null && usable.length){
    if (lmMean > 70){ verdict = 'quite rough'; cloudCap = 'heavy'; }
  ...
  let qualifier = null;
  if (cloudCap === 'heavy') qualifier = 'clouded out for most of the night';
  else if (cloudCap === 'broken') qualifier = 'too much low cloud for uninterrupted shots';
...
function fogRisk(n){
  /* Same rule as dew: the morning side does not count. Fog rolling in as the sky brightens is not
     something to warn a photographer about for a night that has finished. */
  const withCloud = n.slots.filter(s => s.cloud);
  let last = -1;
  withCloud.forEach((s, i) => { if (s.sunAlt < -15) last = i; });
  let dark = last >= 0 ? withCloud.slice(0, last + 1).filter(s => s.sunAlt < -6) : [];
  if (!dark.length) dark = withCloud.filter(s => s.sunAlt < -6);
...
function auroraGeometry(kp, lat, lon){
  ...
  /* The elevation in degrees inherits ARC_OFFSET, which is one number from my own experience and
     has never been checked against a sightings archive. So the app stops quoting it and describes
     the height in words instead: no more precise than the input deserves, and a wrong offset can
     now only move the answer by one band. */
  const heightWord = gap <= 0 ? 'overhead'
    : high < 3 ? 'on the horizon'
    : high < 10 ? 'low on the horizon'
    : high < 25 ? 'mid-height'
    : 'high in the sky';
  return {
    mlat, edge: limit, arcLat, gap, heightWord,
    overhead: gap <= 0,
...
/* The Bortle figure the aurora model should use. Not your own site, which is what the darkness
   readout elsewhere means, but the sky you are actually looking through to see an arc: sixty per
   cent the poleward horizon, forty per cent overhead. Combined in brightness rather than in
   magnitudes, because light adds and magnitudes do not. The sector sampling above already weights
   near glow far above distant glow, which is the right way round: what beats you is the town up
   the road, not the city eighty miles away. */
function auroraGlowBortle(g){
  g = g || polewardGlow();
  if(!g) return null;
  const site = g.zenithSqm, horiz = g.meanSqm;
  if(site == null && horiz == null) return null;
  if(site == null) return bortleFor(horiz);
  if(horiz == null) return bortleFor(site);
  return bortleFor(nl2mag(0.4 * mag2nl(site) + 0.6 * mag2nl(horiz)));
}
...
const AURORA_CAL = {
  ...
  HP_FRESH_MIN: 45,
  V_DEFAULT: 450,
  /* NEWELL_TO_G is gone. Mapping the coupling function onto index units let the next forty
     minutes be drawn on the same axis as the rest of the night, but the table behind it had no
     calibration whatsoever, which made it the weakest link in the tab. The wind's lead time now
     shows as the driving state and as plain text, and nothing plotted on the chart is unsourced. */
};

/* ---- Hp30: the measured half-hourly index, with estimated Kp as the understudy ----
   GFZ publish Hp30, and they do not send the header a browser needs before it will hand a
   cross-origin response to the page. Tested every endpoint they have on 18 August 2026: all of
   them answer, and all of them are unreadable from here. So the page cannot fetch it directly
   however much we would like to, and it needs its own copy of the file re-served with that
   header. worker/hp30.js is that copy, about forty lines on Cloudflare.

   Leave HP30_RELAY null and everything below falls through to NOAA's minute estimate exactly as
   it does today, which is why this is safe to ship before the relay exists. Setting it is a
   one-line change. */
...
async function loadHp30(){
  try{
    ...
    const r = await fetch(HP30_RELAY, { cache: 'no-store' });
    if(!r.ok) throw new Error('HTTP ' + r.status);
    const j = await r.json();
    const series = [];
    (j.series || []).forEach(row => {
      const t = Date.parse(row.t), v = Number(row.hp30);
      if(isFinite(t) && isFinite(v) && v >= 0) series.push({ t, v, ap: Number(row.ap30) });
    });
...
function hpNow(blockKp){
  ...
  /* Hp30 is quantised to thirds, exactly like Kp, so a genuine slow build arrives as a staircase
     rather than a ramp and two equal readings in a row are completely normal mid-build. Reading
     the direction off one pair of samples would call that flat and miss the thing we most want to
     catch. So the trend is a least-squares slope across the last four half hours, two hours of
     history, expressed per hour: one step of a third over two hours stays flat, two steps counts
     as rising. The same maths serves the NOAA fallback, which is bucketed to half hours above for
     this reason. */
  const win = s.slice(-4);
  let dir = 'flat', slope = null;
  if(win.length >= 3){
    const n = win.length, mx = (n - 1) / 2;
    const my = win.reduce((a, p) => a + p.v, 0) / n;
    let num = 0, den = 0;
    win.forEach((p, i) => { num += (i - mx) * (p.v - my); den += (i - mx) * (i - mx); });
    slope = den ? (num / den) * 2 : null;          // index units per hour
    if(slope != null) dir = slope >= 0.25 ? 'rising' : slope <= -0.25 ? 'falling' : 'flat';
  }else if(d1 != null){
    dir = d1 >= 0.5 ? 'rising' : d1 <= -0.5 ? 'falling' : 'flat';
  }
  return {
    value: v, prev, prev2, delta: d1, d2, slope, dir,
...
function windDerived(){
  ...
  return {
    ...
    gDriver: null,   // deliberately not derived: see the note on NEWELL_TO_G above
...
/* ---- is somewhere nearby better tonight? ----
   "100km" means nothing to someone deciding whether to drive: there is no routing engine here,
   no key, no account, so the radius is a straight-line approximation from the chosen drive time,
   discounted for the fact that roads are never straight and always have junctions. 50 km/h of
   genuine progress on a mixed rural and trunk-road route is a reasonable UK average, which is
   where these three numbers come from. Call it an estimate rather than a promise. */
const DRIVE_RADIUS_KM = { 30: 25, 60: 50, 120: 100 };

/* Two rings rather than one, so a nearer, nearly-as-good spot is not overlooked in favour of one
   right at the edge of the drive-time budget, and eight bearings, which is coarse enough to stay
   one polite batched request and fine enough to say a direction with a straight face. Cloud is the
   only thing scanned: light pollution and moon are shared across a spot this size, or coarser than
   this app can resolve outside north-west Europe, so pretending to score them would be the kind of
   invented precision the rest of this file tries to avoid. */
async function loadNearbyScan(radiusKm, darkWin){
  const pts = [];
  [radiusKm * 0.5, radiusKm].forEach(r => {
    COMPASS16.forEach((name, i) => {
      if(i % 2) return;                       // 8 of the 16, N/NE/E/SE/S/SW/W/NW
      pts.push({ ...destPoint(state.lat, state.lon, i * 22.5, r), brg: i * 22.5, km: r });
    });
  });
  const url = 'https://api.open-meteo.com/v1/forecast?latitude=' + pts.map(p => p.lat.toFixed(3)).join(',') +
    '&longitude=' + pts.map(p => p.lon.toFixed(3)).join(',') +
    '&hourly=cloud_cover_low,cloud_cover_mid&forecast_days=2&timezone=UTC';
  try{
    const r = await fetch(url);
    if(!r.ok) throw new Error('HTTP ' + r.status);
    const j = await r.json();
    ...
    const points = rows.map((p, i) => {
      const h = p.hourly, meta = pts[i];
      if(!h || !h.time) return { ...meta, clear: null };
      let sum = 0, n = 0;
      h.time.forEach((t, k) => {
        const ms = Date.parse(t + ':00Z');
        if(darkWin && (ms < darkWin.from.getTime() - 1800000 || ms > darkWin.to.getTime() + 1800000)) return;
        const lo = h.cloud_cover_low[k], mid = h.cloud_cover_mid[k];
        if(lo == null && mid == null) return;
        sum += Math.max(lo || 0, mid || 0); n++;
      });
      return { ...meta, clear: n ? 100 - sum / n : null };
    });
    state.nearby = { at: Date.now(), radiusKm, points };
    state.nearbyStatus = 'live';
    return true;
  }catch(e){ state.nearby = null; state.nearbyStatus = 'unavailable'; return false; }
}

/* ---- tide ----
   Two different jobs, two different sources, deliberately kept apart.

   Proximity is free: whether to show the tile at all is decided from the National Tide Gauge
   Network list, 43 real stations spanning the whole UK and Irish coastline, fetched live from the
   Environment Agency's flood-monitoring API on 18 August 2026 and baked in here because a list of
   fixed points on a coastline does not move. It is also what the free fallback reading below uses.

   A forecast is not free: nobody publishes predicted UK tide times without either an account or a
   licence, so the actual "high water at 22:14" answer comes from the Admiralty Tidal API through a
   relay of my own, on the same shape as the Hp30 one. Until that relay is set, the tile still shows
   something real and useful, the most recent measured level, just not a time ahead of now. */
const TIDE_STATIONS = [
  ['Lowestoft',52.4731,1.7501,'E70039'],['Avonmouth Portbury',51.5,-2.7285,'E72639'],
  ['Sheerness',51.4456,0.7434,'E71539'],['Bournemouth',50.7143,-1.8749,'E71939'],
  ['Cromer',52.9343,1.3016,'E71239'],['Weymouth',50.6085,-2.4479,'E72039'],
  ['Dover',51.1144,1.3226,'E71639'],['Heysham',54.0318,-2.9203,'E73439-anglian'],
  ['Ilfracombe',51.2111,-4.1124,'E72439'],['North Shields',55.0074,-1.4398,'E70939'],
  ['Workington',54.6507,-3.5672,'E73639-anglian'],['Plymouth',50.3684,-4.1852,'E72139'],
  ['Newhaven',50.7818,0.057,'E71739'],['Whitby',54.49,-0.6146,'E71039'],
  ['Liverpool',53.4497,-3.0181,'E70139-anglian'],['Hinkley Point',51.2106,-3.1313,'E72539'],
  ['Harwich',51.948,1.2921,'E71439'],['Newlyn',50.103,-5.5428,'E72239'],
  ['Portsmouth',50.8023,-1.1112,'E71839'],['Immingham',53.6302,-0.1874,'E71139'],
  ['Washpile',52.8759,0.2184,'E71339'],['St Marys',49.9179,-6.3172,'E72324'],
  ['Portrush',55.2011,-6.6512,'E73939'],['Holyhead',53.3139,-4.6204,'E70539'],
  ['Fishguard',52.0132,-4.9837,'E73139'],['Ullapool',57.8953,-5.1579,'E70439'],
  ['Leith',55.9898,-3.1817,'E70824'],['Mumbles',51.57,-3.9754,'E72924'],
  ['Newport',51.55,-2.9874,'E72839'],['Aberdeen',57.1441,-2.0801,'E70739'],
  ['Milford Haven',51.7074,-5.0518,'E73024'],['Millport',55.7498,-4.9063,'E74039'],
  ['Bangor',54.6652,-5.6704,'E73839'],['Wick',58.441,-3.0863,'E70624'],
  ['Port Erin',54.0854,-4.7681,'E73524'],['Kinlochbervie',58.4567,-5.0502,'E70339'],
  ['Tobermory',56.6231,-6.0642,'E74224'],['Portpatrick',54.8425,-5.12,'E73739'],
  ['Stornoway',58.2078,-6.389,'E74324'],['Jersey',49.1833,-2.1167,'E70239'],
  ['Barmouth',52.7193,-4.045,'E73224'],['Lerwick',60.154,-1.1403,'E74424'],
  ['Llandudno',53.3316,-3.8252,'E73324'],
];
function nearestTideStation(lat, lon){
  lat = lat == null ? state.lat : lat; lon = lon == null ? state.lon : lon;
  let best = null;
  TIDE_STATIONS.forEach(s => {
    const dLat = s[1] - lat, dLon = (s[2] - lon) * Math.cos(lat * Math.PI / 180);
    const km = Math.sqrt(dLat * dLat + dLon * dLon) * 111.195;
    if(!best || km < best.km) best = { name: s[0], lat: s[1], lon: s[2], ref: s[3], km };
  });
  return best;
}
...
/* Moonlight, from the published sky-brightness model the rest of the app already runs, rather
   than from two multipliers I invented. Evaluated low towards the poleward horizon, where the
   aurora actually is, instead of at the zenith. The eye still loses far more than the sensor,
   but that now falls out of the physics: the divisors below are simply where a dark-adapted eye
   and a long exposure each give up, and the old 0.50 and 0.18 sit on this curve at about the
   moon that produced them. */
function moonHitAurora(env, poleward){
  if(!env || env.moonAlt == null || env.moonAlt <= -0.8) return { eye: 0, camera: 0, loss: 0 };
  const lm = limitingMag(env, 12, poleward === 'south' ? 180 : 0);
  const loss = Math.max(0, lm.moonLoss || 0);
  return { eye: clamp(loss / 2.2, 0, 0.85), camera: clamp(loss / 6.0, 0, 0.42), loss };
}
...
function chanceAt(level, opts){
  ...
  if(opts.glow !== false){
    const b = auroraGlowBortle(opts.glow || null);
    if(b != null) bortle = b;
  }
  if(bortle == null) bortle = 4;
  const mh = opts.env
...
/* The likelihood ladder. Five rungs rather than four, so an error in any one constant moves the
   ...
   speech, since possible and likely are not really points on one scale, so wherever these words
   appear the app shows the ladder itself rather than assuming the reader shares my ranking. */
const CHANCE_LADDER = ['unlikely', 'possible', 'very possible', 'likely', 'very likely'];
function chanceWord(p){
  if(p == null) return 'no reading';
  if(p >= 0.8) return 'very likely';
  if(p >= 0.62) return 'likely';
  if(p >= 0.44) return 'very possible';
  if(p >= 0.28) return 'possible';
  return 'unlikely';
}
function wordRung(w){ return CHANCE_LADDER.indexOf(w); }
/* Confidence rather than likelihood. Nights beyond tonight, and anything drawn from the 27-day
   outlook, must not reach the top of the ladder however good the numbers look, because the
   uncertainty is in the forecast rather than in the sky. */
function capWord(w, maxRung){
  const i = CHANCE_LADDER.indexOf(w);
  return i < 0 ? w : CHANCE_LADDER[Math.min(i, maxRung)];
}
function liftWord(w, rungs){
  const i = CHANCE_LADDER.indexOf(w);
  return i < 0 ? w : CHANCE_LADDER[Math.min(CHANCE_LADDER.length - 1, i + (rungs || 0))];
}
...
function nightCurve(win, kpSeries, opts){
  ...
  for(let t = from; t <= to; t += 15 * 60000){
    ...
    if(t <= now){
      ...
      /* The wind segment is no longer plotted as an index level: gDriver is gone with the
         uncalibrated coupling table. What the forty minutes of real lead time buys us now is the
         driving state and the note beside the chart, not a line pretending to be a measurement.
         So the curve runs measured, then blends into the forecast on the persistence half life. */
      const base = eNow;
      const w = Math.exp(-(t - now) / (AURORA_CAL.TAU_PERSIST_MIN * 60000));
      const f = kpAt(t);
      level = f == null ? base : (base == null ? f : w * base + (1 - w) * f);
      source = t <= arrival ? 'live' : (w > 0.5 ? 'live' : 'forecast');
    }
...
/* ---- driving against response ----
   ...
   hard south with it falling is the gap between two of those.
   There used to be a third reading here, "loaded": hard driving with the ground index flat, on the
   reasoning that energy was going into the tail and had not come out yet. It has been dropped. Two
   things were wrong with it. A flat index under hard driving does not reliably mean loading rather
   than nothing much happening, and the trend itself is read from a three-hour smoothed estimate
   (see hpNow), so a flat reading may be an artefact of the data source rather than of the sky.
   Building and easing are descriptions. Loaded was an interpretation, and not one I could source. */
function couplingState(){
  ...
  if(driving === 'hard' && dir === 'flat') return {
    key: 'driving', label: 'Driving hard', urgent: true,
    line: 'The wind has been pushing the right way for hours, but nothing has shown on the ground yet. That is worth watching rather than acting on: it may break, or it may not.',
  };
