...
/* Grids baked from David Lorenz's 2025 light pollution atlas, itself modelled from
   ...
   PENDING: the scandi grid is referenced by the lookup order but its pixel data is not baked in
   yet, so Scandinavia currently falls back to the world grid. Notes on how it was being built:
   the scandi grid is NOT a direct export: Lorenz's site only offers a colour-rendered continent
   image, not the underlying numbers. It was rebuilt by reading that image's colours against the
   trusted uk grid, which the same image also covers, and letting that overlap teach a colour-to-
   brightness table (144,000 real training pixels, held-out check: mean error 0.02 mag, 95th
   percentile 0.1 mag). Reasonable rather than Solid: a genuine reconstruction, cross-checked
   against real data, but not the atlas's own figures. */
const ATLAS={
