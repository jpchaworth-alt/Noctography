...
uniform float uGain; uniform float uHazeM; uniform float uHaze; uniform float uNv;
varying vec2 vUv; varying vec3 vNrm; varying float vDist;
void main(){
...
function makeGL(canvas){
  ...
  const u = {};
  ['uMVP', 'uTex', 'uSun', 'uMoon', 'uSunAlt', 'uMoonLight', 'uGain', 'uHazeM', 'uHaze', 'uNv'].forEach(k => u[k] = gl.getUniformLocation(prog, k));
...
class Scene {
  ...
  /* Draw. view: { az, alt, hfov, vfov, eyeM }; light: { sunAz, sunAlt, moonAz, moonAlt, moonFrac, gain, hazeKm, haze, nv } */
  render(view, light){
...
function loadBand(src, onReady){
  ...
  img.onload = () => {
    try {
      ...
      for (let i = 0; i < 256; i++) {
        const x = Math.max(0, (i - B0) / (255 - B0));
        lut[i] = Math.min(1, Math.pow(x, 0.58) * 1.04) * 255;
      }
...
async function loadSkyData(onReady){
  ...
  try {
    ...
    SKY.stars = (st.features || []).map(f => ({ ra: ra(f.geometry.coordinates[0]), dec: f.geometry.coordinates[1], mag: Number(f.properties.mag) })).filter(s => isFinite(s.mag));
...
function drawSky(ctx, W, H, view, when, lat, lon, opts){
  ...
  if (dark > 0) {
    ...
    sky.stars.forEach(s => {
      ...
      const scale = Math.min(2.2, Math.max(0.6, pr.kx / (W / 2) / 1.2));   // tighter lens, bigger stars
...
window.NoctoScout = {
  ...
  loadSkyData, skyDataReady: () => !!SKY.stars, skyDataFailed: () => SKY.failed,
