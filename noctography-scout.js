  ...
  async load(lat, lon, onProgress, onRing){
    ...
    await Promise.all(p.rings.map(async (r, i) => {
      ...
      if (i === 0) {
        ...
        /* The eye stands on the surface the mesh actually draws, not on one height sample: on a
           steep slope the nearest post can sit metres below the triangles around it, which put
           the camera inside the hill and the ground all over the screen. The highest of the posts
           around the pin keeps it above every triangle it could be standing on. */
        const gi = Math.max(0, Math.min(W - 2, Math.floor(fx))), gj = Math.max(0, Math.min(W - 2, Math.floor(fy)));
        const h = (x, y) => data.heights[y * W + x] || 0;
        this.ground = Math.max(h(gi, gj), h(gi + 1, gj), h(gi, gj + 1), h(gi + 1, gj + 1));
      }
...
let bandLifted = null, bandLoading = false;
function loadBand(src, onReady){
  ...
  img.onload = () => {
    try {
      /* 2048 across is five pixels a degree, all the panorama strip draws at, and a quarter of the
         memory of 4096: on a phone that matters more than detail nobody can see */
      const coarse = window.matchMedia && window.matchMedia('(pointer: coarse)').matches;
      const w = Math.min(coarse ? 2048 : 4096, img.naturalWidth), h = Math.round(w / 2);
      ...
      bandLifted = cv;
      img.src = '';   // the 8k decode is 134 MB; the lifted copy is all anything uses
...
/* a few days at once, for the endless strip in Sky AR: the one either side of the day in view */
...
/* Paint the strip. o: { minutes, baseMs, lat, lon, events, nv, accent, ink, liveMinutes,
   infinite, eventsFor(k) }. With infinite set, minutes may run past either end of the day and
   the neighbouring days are drawn in, each with its own sky colour and its own marks. */
function paintStrip(cv, o){
  ...
  const mLo = o.minutes - W / 2 / ppm, mHi = o.minutes + W / 2 / ppm;
  const k0 = o.infinite ? Math.floor(mLo / 1440) : 0, k1 = o.infinite ? Math.floor(mHi / 1440) : 0;
  for (let k = k0; k <= k1; k++) {
    const bg = stripBackground(o.baseMs + k * 86400000, o.lat, o.lon, dpr);
    if (!bg) continue;
    if (nv) { ctx.save(); ctx.filter = 'grayscale(1) sepia(1) saturate(6) hue-rotate(-32deg) brightness(.7)'; }
    ctx.drawImage(bg, 0, 0, bg.width, 8, xOf(k * 1440), 0, bg.width, H);
    if (nv) ctx.restore();
  }
  ...
  const mA = o.infinite ? Math.floor(mLo / 30) * 30 - 30 : 0, mB = o.infinite ? mHi + 30 : 1440;
  for (let m = mA; m <= mB; m += 30) {
    const x = xOf(m); if (x < -20 || x > W + 20) continue;
    const hour = ((m % 60) + 60) % 60 === 0;
    const hh = (((12 + m / 60) % 24) + 24) % 24;
    ctx.fillStyle = inkA(hour ? 0.55 : 0.22);
    ctx.fillRect(x - 0.5 * dpr, 0, dpr, (hour ? 10 : 5) * dpr);
    /* midnight carries the new day's name, so a long scroll always says where it has got to */
    if (o.infinite && hour && hh === 0) { ctx.fillStyle = inkA(0.4); ctx.fillRect(x - 0.5 * dpr, 0, dpr, H); }
    if (hour) {
      const label = o.infinite && hh === 0
        ? (() => { const d = new Date(o.baseMs + m * 60000); return DAYS[d.getDay()] + ' ' + d.getDate(); })()
  ...
  const placed = [], evs = [];
  for (let k = k0; k <= k1; k++) {
    const list = o.eventsFor ? o.eventsFor(k) : (k === 0 ? o.events : null);
    (list || []).forEach(ev => evs.push(k ? Object.assign({}, ev, { m: ev.m + k * 1440 }) : ev));
  }
  evs.forEach(ev => {
...
function stripGestures(el, h){
  let s = null, coast = 0;
  const clampM = h.infinite ? (m => m) : (m => Math.max(0, Math.min(1439, m)));
  const down = e => {
    /* a control sitting on the strip (Now) keeps its own taps */
    if (e.target && e.target.closest && e.target.closest('[data-strip-skip]')) return;
  ...
  const up = e => {
    ...
    const tick = () => {
      ...
      if (Math.abs(v) < 0.02 || (!h.infinite && (m <= 0 || m >= 1439))) { coast = 0; h.commit(m); return; }
      h.set(m, true);
...
/* ---------------- the panorama strip ----------------
   ...
   slice's camera, so the strip is exact rather than a mosaic of flat frames. Forty degrees below
   the horizon, for a mountain top where half the picture is the drop in front of you. */
const PANO_TOP = 90, PANO_BOT = -40;
...
async function panoStrip(scene, out, o){
  ...
  /* the horizon band first, then the sky, then the ground below: the part most compositions are
     built on arrives first, and the drop below a summit last. groundOnly draws no sky at all and
     keeps the ground's own alpha, for the skyline chart. */
  const tiers = o.tiers || [{ e: 0, lo: -12, hi: 40, vh: 42 }, { e: 64, lo: 40, hi: PANO_TOP, vh: 30 }, { e: -38, lo: PANO_BOT, hi: -12, vh: 30 }];
  ...
  for (const T of tiers) {
    ...
    for (let j = 0; j < N; j++) {
      ...
      const src = tctx.getImageData(0, 0, sw, sh).data, keepA = !!o.groundOnly;
      for (let x = 0; x < SW * PPD; x++) {
        ...
        for (let y = y0; y < y1; y++) {
          ...
          od[oi] = src[si]; od[oi + 1] = src[si + 1]; od[oi + 2] = src[si + 2]; od[oi + 3] = keepA ? src[si + 3] : 255;
        }
      }
