...
async function panoStrip(scene, out, o){
  ...
  const PPD = o.ppd || 5, k = PPD * R2D, half = 10;
  ...
  for (const T of tiers) {
    ...
    for (let i = 0; i < 18; i++) {
      ...
      const view = { az: i * 20 + half, alt: T.e, hfov, vfov, eyeM: o.eyeM || 1.6 };
      ...
      for (let x = 0; x < 20 * PPD; x++) {
        const d = ((x + 0.5) / PPD - half) * D2R, sd = Math.sin(d), cd = Math.cos(d), ox = i * 20 * PPD + x;
        for (let y = y0; y < y1; y++) {
    ...
    /* the ground and low sky first, so there is something to compose on while the rest draws */
